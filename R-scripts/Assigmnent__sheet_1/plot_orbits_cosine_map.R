...
# cosine map function
cosine_map <- function(mu, x0, n) {
  x_values <- numeric(n)
  x_values[1] <- x0
  for (i in 2:n) {
    x_values[i] <- mu * cos(x_values[i-1])
  }
...
x0_values <- c(-0.5, 0.0, 0.7)  # Different initial conditions
mu_values <- c(1.0, 1.6, 1.85, 2.0)  # Different growth rates

# Generate orbits for different values of r and x0
orbit_data <- data.frame()
for (mu in mu_values) {
  for (x0 in x0_values) {
    orbit_data <- rbind(orbit_data, cosine_map(mu, x0, n))
  }
}
...
ggplot(orbit_data, aes(x = n, y = x, color = factor(mu))) +
  ...
  xlab("Iteration (n)") +
  ylab("x_n") +
  ggtitle("Orbits of the cosine map for different values of mu and x0") +
  scale_color_discrete(name = "mu values") +
  theme_minimal()


png("Figures_videos/Assignment_sheet_1/orbits_cosine_map.png",
