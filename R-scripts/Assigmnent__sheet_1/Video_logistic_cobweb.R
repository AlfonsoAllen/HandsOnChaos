...
cobweb_plot <- function(mu, x0 = 0.2, n_iter = 100) {
  ...
  p <- ggplot() +
    ...
    geom_segment(data = cobweb_df, aes(x = x, xend = x, y = x, yend = y), color = "red") +  # Vertical segment
    geom_segment(data = cobweb_df, aes(x = x, xend = y, y = y, yend = y), color = "red") +  # Horizontal segment
...
output_video <- "Figures_videos/Assignment_sheet_1/video_cobweb_logistic_map.mp4"
