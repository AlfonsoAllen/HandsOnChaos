...
cobweb_plot <- function(r, x0 = 0.2, n_iter = 100) {
  x <- numeric(n_iter)
  ...
  p <- ggplot() +
    ...
    labs(title = paste("Cobweb Plot (r =", round(r, 2), ")"),
         x = "x_n", y = "x_(n+1)") +
    theme_minimal()
...
output_video <- "logistic_cobweb.mp4"

# Vector with the values of r
r_values <- seq(0, 4, by = 0.01)  # More gradual changes
...
av::av_capture_graphics({
  for (i in 1:length(r_values)) {
    ...
    cobweb_plot(r = r_values[i], x0 = 0.2, n_iter = 500)
