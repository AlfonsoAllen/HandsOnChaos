...
To display `x` and check it just type:

   ```{r}
    x
   ```
   
You can then use the variable in yourcalculations:
...
### 3.2 Define a function for a straight line:
   ...
   ```
### 3.3 Apply these functions to a sequence of values and store results.
...
5.2.2. Find a root in a given interval (for example, $[0, 2]$):
   ```{r}
   root_f <- uniroot(dx_dt, interval = c(0, 2))$root
   print(root_f)
   ```
