...
You can also extract:
   ...
   ```
So leaving an index empty means "select everything" in that dimension.
...
We first define a function that computes the value of a parabola of the form  $y = x^2$.
...
- `parabola` is the name of the function

- `x` is the input variable
- `x^2` is the operation performed on the input
- `return()` specifies the value that the function outputs

### 3.2 Define a function for a straight line

Now lets define a function for a straight line of the form $y=mx+b$.
...
- `x` is the input variable
- `m` is the slope of the line (default value: 1)
- `b` is the intercept (default value: 0)

The default values mean that if m and b are not specified, the function computes $y=x$.
