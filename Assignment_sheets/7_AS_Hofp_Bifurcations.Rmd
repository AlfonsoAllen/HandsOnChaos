...
# Simulations (mu < 0)
sol_1_neg_mu_sub <- simulate_hopf_sub(state1, parameters_neg_mu, times)
...
Hence, at $\mu =-.01$, there is one unstable limit cycle at the smaller radius and one stable limit cycle at the larger radius. The trivial solution $r=0$ also exists, but its stability typically depends on  $\mu$ and the interplay with the other cycles (in a subcritical Hopf scenario, the origin can be stable for certain ranges of $\mu$, leading to bistability).
