---
title: "1D-Bifurcations (Assignment Sheet 5)"
...
A **supercritical bifurcation** occurs when a stable equilibrium splits into two new stable equilibria as the control parameter \( r \) crosses a critical value. The system under study is:
...
ggplot(data = dx_dt_df, 
  ...
  labs(title = "Supercritical Bifurcation for dx/dt = r x - x^3", 
       x = "x", y = "dx/dt", color = "r") +
  facet_grid(~r)+
  theme_bw()
```
...
ggplot(bifurcation_data, aes(x = r, y = x, color = stability)) +
  ...
  labs(title = "Supercritical Bifurcation Diagram",
...
# 2. Subcritical bifurcation

## **2.1 Theoretical background**

A **subcritical bifurcation** is characterized by an unstable fixed point that merges with a stable one as the control parameter \( r \) changes. The system under study is:
...
ggplot(bifurcation_data, aes(x = r, y = x, color = stability)) +
  geom_point(size = 1.5, alpha = 0.7) +
  scale_color_manual(values = c("Stable" = "blue", "Unstable" = "red")) +
  labs(title = "Subcritical Bifurcation Diagram", 
       x = "r (Control Parameter)", y = "Fixed Points") +
  theme_bw()
```
...
Finally, we construct the bifurcation diagram by identifying the fixed points across a range of \( r \) values and coloring them according to their stability. Notice how, for \( r > 0 \), two branches appear (stable and unstable), and for \( r < 0 \), there is no real fixed point.
...
for (r in r_values) {
  ...
  for (x in points) {
    bifurcation_data <- rbind(bifurcation_data, data.frame(r = r, x = x, stability = stability(x, r)))
  }
}
...
This pactice exercise is focused on characterizing the **subcritical bifurcation** of the system:
...
3. **Sketch Phase Portraits**  
   ...
   - Plot \( \dot{x} \) vs. \( x \) by hand or in R, identify where \( \dot{x} = 0 \), and mark equilibria as stable or unstable.

4. **Numerical Bifurcation Diagram**  
   - Adapt or run the R script below, which uses polynomial root-finding (`polyroot`) to locate equilibria.  
   - Color them by stability and plot them against \( r \).

5. **Interpret the Results**  
   - Identify the critical parameter value(s) of \( r \) that cause a subcritical bifurcation.  
